...
final class ShellTickSmokeTests: XCTestCase {
    ...
    func testThrownDaggerUnequipsAndSaveRemainsRejectedInCombat() throws {
        ...
        let traceCommand = renderCommand(in: state.renderFrame, index: Int(state.renderFrame.command_count) - 2)
        let flashCommand = renderCommand(in: state.renderFrame, index: Int(state.renderFrame.command_count) - 1)
        XCTAssertEqual(traceCommand.kind, UInt8(U3_RENDER_COMMAND_RECT))
        XCTAssertEqual(traceCommand.x, 5)
        XCTAssertEqual(traceCommand.y, 5)
        XCTAssertEqual(traceCommand.width, 5)
        XCTAssertEqual(flashCommand.kind, UInt8(U3_RENDER_COMMAND_RECT))
        XCTAssertEqual(flashCommand.x, 9)
        XCTAssertEqual(flashCommand.y, 5)
    ...
    func testMittarSpellSpendsMagicAndSaveRemainsRejectedInCombat() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setActiveRosterClass(rosterID: 1, characterClass: UInt8(ascii: "W"), in: &document))
        XCTAssertTrue(setActiveRosterMagic(rosterID: 1, magic: 25, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        )
        ...
        state.submitKeyboard(UInt8(ascii: "M"))
        state.runTick()
        XCTAssertEqual(state.lastCommand, "Combat character 1 Mittar: choose direction")
        state.submitKeyboard(UInt8(ascii: "E"))
        state.runTick()

        XCTAssertEqual(state.lastCommand, "Combat character 1 Mittar monster 0 damage 21 magic 20 | Monster 1 move 7,4 | Audio Sound Immolate")
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: currentDocument), 20)
        XCTAssertTrue(state.hasUnsavedChanges)
    ...
    func testFulgarSpellPromptsSpendsMagicAndUsesFixedDamage() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setActiveRosterClass(rosterID: 1, characterClass: UInt8(ascii: "W"), in: &document))
        XCTAssertTrue(setActiveRosterMagic(rosterID: 1, magic: 30, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        )
        ...
        state.submitKeyboard(UInt8(ascii: "U"))
        state.runTick()
        XCTAssertEqual(state.lastCommand, "Combat character 1 Fulgar: choose direction")
        state.submitKeyboard(UInt8(ascii: "E"))
        state.runTick()

        XCTAssertEqual(state.lastCommand, "Combat character 1 Fulgar monster 0 damage 75 magic 5 | Monster 1 move 7,4 | Audio Sound Immolate")
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: currentDocument), 5)
        XCTAssertTrue(state.hasUnsavedChanges)
    ...
    func testNoxumSpellDoesNotPromptAndUsesBigDeathFeedback() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setActiveRosterClass(rosterID: 1, characterClass: UInt8(ascii: "W"), in: &document))
        XCTAssertTrue(setActiveRosterMagic(rosterID: 1, magic: 55, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        )
        ...
        state.submitKeyboard(UInt8(ascii: "O"))
        state.runTick()

        XCTAssertEqual(state.lastCommand, "Combat character 1 Noxum damaged 1 defeated 0 magic 5 | Monster 1 move 7,4 | Audio Sound BigDeath")
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: currentDocument), 5)
        XCTAssertTrue(state.hasUnsavedChanges)
    ...
    func testMittarVictoryAppliesExperienceAndPersistsAfterReturn() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        ...
        let state = ShellSmokeState(
            ...
            combatPlayerRollProvider: { _ in (hitChance: 128, hitDexterity: 0, damage: 5) },
            combatVictoryRewardProvider: { (gold: 75, weapon: 5, armour: 3) },
            combatMonsterRollProvider: {
                (
                    shootChoice: 255,
                    magicChoice: 0,
        ...
        session.combatState.monster_x.0 = 8
        ...
        session.combatState.character_class.0 = UInt8(ascii: "W")
        session.combatState.character_magic.0 = 25
        ...
        withUnsafeMutableBytes(of: &session.combatState.tile_array) { buffer in
            ...
            buffer[(4 * 11) + 8] = session.monsterType
        ...
        XCTAssertEqual(
            state.lastCommand,
            "Combat character 1 Mittar monster 0 defeated damage 21 magic 20 exp 6 | Combat victory remaining 0 reward character 1 gold 225 added 75 weapon 5 qty 1 armour 3 qty 1 returned dungeon MAPS 412 | Audio Sound Immolate"
        )
        ...
        state.saveGame()
        XCTAssertTrue(state.saveStatus.hasPrefix("Game saved | "))
        state.loadGame()
        let updatedDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: updatedDocument), 20)
        XCTAssertEqual(activeRosterExperience(rosterID: 1, in: updatedDocument), 6)
    }

    func testMittarInsufficientMagicConsumesTurnWithoutMutation() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setActiveRosterClass(rosterID: 1, characterClass: UInt8(ascii: "W"), in: &document))
        XCTAssertTrue(setActiveRosterMagic(rosterID: 1, magic: 4, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        )
        ...
        XCTAssertEqual(state.lastCommand, "Combat character 1 Mittar insufficient magic 4 | Monster 1 move 7,4 | Audio Sound FailedSpell")
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: currentDocument), 4)
    }

    func testMittarInvalidCasterDoesNotSpendMagicOrConsumeTurn() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setActiveRosterClass(rosterID: 1, characterClass: UInt8(ascii: "F"), in: &document))
        XCTAssertTrue(setActiveRosterMagic(rosterID: 1, magic: 25, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        ...
        XCTAssertEqual(state.lastCommand, "Combat character 1 Mittar invalid caster | Audio Sound FailedSpell")
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
        XCTAssertEqual(activeRosterMagic(rosterID: 1, in: currentDocument), 25)
    }

    func testCombatFleeSuccessRestoresDungeonAndRejectsSave() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        ...
        let state = ShellSmokeState(
            locationProvider: locationProvider,
            dungeonRollProvider: { _ in (encounter: 128, monster: 2) },
            combatFleeRollProvider: { UInt8(U3_COMBAT_FLEE_SUCCESS_THRESHOLD) }
        )
        state.loadGame()
        state.submitKeyboard(UInt8(ascii: "E"))
        state.runTick()
        ...
        state.submitKeyboard(UInt8(ascii: "F"))
        state.runTick()

        XCTAssertEqual(
            state.lastCommand,
            "Combat character 1 flee succeeded | Combat fled roll 128 returned dungeon MAPS 412 | Audio Sound Step"
        )
        XCTAssertNil(state.debugActiveCombatStatus())
        XCTAssertTrue(state.resourceStatus.contains("Dungeon OK MAPS 412 level 0 pos 1,1 heading 1 light 0"))
        XCTAssertTrue(state.hasUnsavedChanges)
    ...
    func testCombatFleeFailureConsumesTurnAndKeepsCombatActive() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        ...
        let state = ShellSmokeState(
            locationProvider: locationProvider,
            dungeonRollProvider: { _ in (encounter: 128, monster: 2) },
            combatFleeRollProvider: { UInt8(U3_COMBAT_FLEE_SUCCESS_THRESHOLD - 1) },
            combatMonsterRollProvider: {
                (
                    shootChoice: 255,
                    magicChoice: 0,
        ...
        XCTAssertEqual(state.lastCommand, "Combat character 1 flee failed roll 127 | Monster 1 move 7,4 | Audio Sound Bump")
        XCTAssertNotNil(state.debugActiveCombatStatus())
        XCTAssertTrue(state.resourceStatus.contains("Combat OK CONS 402"))
    }
    ...
    func testCombatMonsterHitMutatesCurrentRosterButSaveRemainsRejected() throws {
        ...
        let feedbackCommand = renderCommand(in: state.renderFrame, index: Int(state.renderFrame.command_count) - 1)
        XCTAssertEqual(feedbackCommand.kind, UInt8(U3_RENDER_COMMAND_RECT))
        XCTAssertEqual(feedbackCommand.x, 5)
        XCTAssertEqual(feedbackCommand.y, 6)
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
    ...
    func testCombatAllPartyDefeatedRestoresDungeonAndRejectsSave() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        XCTAssertTrue(setPartySize(1, in: &document))
        XCTAssertEqual(
            ShellSaveAdapter().writeDocument(document, saveDocumentPath: saveDocumentURL.path),
            .saved
        )
        ...
        XCTAssertEqual(
            state.lastCommand,
            "Combat character 1 pass | Monster 1 hit character 1 damage 5 defeated | Combat defeat active 0 defeated 1 returned dungeon MAPS 412 | Audio Sound Hit"
        )
        XCTAssertNil(state.debugActiveCombatStatus())
        XCTAssertTrue(state.resourceStatus.contains("Dungeon OK MAPS 412 level 0 pos 1,1 heading 1 light 0"))
        let currentDocument = try XCTUnwrap(state.debugCurrentSaveDocument())
    ...
    func testCombatVictoryRestoresDungeonAndExperiencePersistsAfterReturn() throws {
        ...
        XCTAssertEqual(
            state.lastCommand,
            "Combat character 1 hit monster 0 defeated damage 19 exp 6 | Combat victory remaining 0 reward character 1 gold 225 added 75 weapon 5 qty 1 armour 3 qty 1 returned dungeon MAPS 412 | Audio Sound Hit"
        )
        XCTAssertNil(state.debugActiveCombatStatus())
        XCTAssertEqual(state.debugCurrentDungeonTile(), 0)
        ...
        let updatedDocument = try Data(contentsOf: saveDocumentURL)
        XCTAssertEqual(activeRosterExperience(rosterID: 1, in: updatedDocument), 6)
        XCTAssertEqual(activeRosterGold(rosterID: 1, in: updatedDocument), 225)
        XCTAssertEqual(activeRosterByte(rosterID: 1, offset: 48 + 5, in: updatedDocument), 1)
        XCTAssertEqual(activeRosterByte(rosterID: 1, offset: 40 + 3, in: updatedDocument), 1)
    ...
    func testDungeonChestTrapMutatesSelectedCharacterAndClearsChest() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        ...
        XCTAssertEqual(state.lastCommand, "interaction chest roster 1 gold 200 added 50 trap 1 damage 8 HP 92 moves 8 passive | Audio Sound Hit")
        XCTAssertEqual(state.debugCurrentDungeonTile(), 0)
        XCTAssertTrue(state.hasUnsavedChanges)
        ...
        XCTAssertEqual(activeRosterHitPoints(rosterID: 1, in: updatedDocument), 92)
        XCTAssertEqual(activeRosterGold(rosterID: 1, in: updatedDocument), 200)
    }

    func testDungeonChestCanPersistWeaponAndArmourRewardsAfterReturn() throws {
        let locationProvider = ShellLocationProvider(saveDocumentURL: saveDocumentURL)
        let adapter = ShellResourceAdapter()
        var document = try XCTUnwrap(adapter.buildNativeNewGameSmokeDocument(resourceRootPath: resourceRootPath))
        XCTAssertTrue(setPartyPosition(x: 19, y: 57, in: &document))
        ...
        var rewardRollCallCount = 0
        let state = ShellSmokeState(
            locationProvider: locationProvider,
            dungeonRollProvider: noDungeonEncounterRolls,
            dungeonChestRollProvider: {
                rewardRollCallCount += 1
                return rewardRollCallCount == 1 ?
                    (trap: 0, gold: UInt16((5 << 8) | 20)) :
                    (trap: 0, gold: UInt16((131 << 8) | 20))
            }
        )
        state.loadGame()
        state.submitKeyboard(UInt8(ascii: "E"))
        ...
        state.submitKeyboard(UInt8(ascii: "1"))
        state.runTick()
        XCTAssertEqual(state.lastCommand, "interaction chest roster 1 gold 200 added 50 weapon 5 qty 1 moves 8 passive | Audio Sound Creak")
        ...
        state.submitKeyboard(UInt8(ascii: "1"))
        state.runTick()
        XCTAssertEqual(state.lastCommand, "interaction chest roster 1 gold 250 added 50 armour 3 qty 1 moves 12 passive | Audio Sound Creak")
    ...
    private func setPartySize(_ partySize: UInt8, in documentData: inout Data) -> Bool {
        let documentLength = documentData.count
        return documentData.withUnsafeMutableBytes { documentBuffer in
            guard let documentBaseAddress = documentBuffer.bindMemory(to: UInt8.self).baseAddress else {
                return false
            ...
            party[Int(U3_OVERWORLD_PARTY_SIZE_OFFSET)] = partySize
            for index in Int(partySize)..<Int(U3_PARTY_ACTIVE_SLOT_COUNT) {
                party[6 + index] = 0
            }
            return true
        }
    ...
    private func setActiveRosterClass(rosterID: UInt8, characterClass: UInt8, in documentData: inout Data) -> Bool {
        setActiveRosterByte(rosterID: rosterID, offset: 23, value: characterClass, in: &documentData)
    }

    private func setActiveRosterMagic(rosterID: UInt8, magic: UInt8, in documentData: inout Data) -> Bool {
        setActiveRosterByte(rosterID: rosterID, offset: 25, value: magic, in: &documentData)
    }
    ...
    private func activeRosterMagic(rosterID: UInt8, in documentData: Data) -> UInt8? {
        activeRosterByte(rosterID: rosterID, offset: 25, in: documentData)
    }
