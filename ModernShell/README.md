...
Keyboard, mouse, and menu actions pass through `ShellInputAdapter`, which translates AppKit-owned events into the portable `u3_input_queue`. The shell tick consumes queued input events and reports the consumed command. Macro command sequences use the same queue in legacy execution order.
...
`ShellLocationProvider` owns the first concrete resource-root and save-document path candidates for the AppKit shell. `ShellResourceAdapter` uses the shell-provided resource root to read `Resources/English.lproj/MainResources.rsrc`, validate known fixture records through the portable `u3_resource` parser, build the resource-backed render smoke frame, build/load the native new-game smoke document, and inspect the portable party/roster summary. `ShellSaveAdapter` stages, validates, atomically replaces, and reads back the save smoke document. The game host status overlay reports command, tick, resource/render, and save-path smoke state; Game > Refresh Locations reruns the location/resource/render smoke, Game > Write Save Smoke runs the save write/read smoke, Game > Load New Game Smoke loads the portable save-domain state, Game > Inspect Party/Roster reports active party and leader roster state, and Game > Toggle Tick pauses or resumes the fixed timer.
