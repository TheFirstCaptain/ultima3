...
- `combat_player_command_tests.c` characterizes the first bounded player combat command contract from `Sources/UltimaSpellCombat.c`: cardinal movement, blocked movement, pass, bounded flee success/failure, attack and directed spell prompting, melee hit/miss handoff, projectile and thrown-dagger line targeting, thrown-dagger inventory mutation, Mittar/Fulgar/Noxum magic spend and damage shapes, spell invalid/insufficient-resource cases, all-party-defeated detection, redraw/audio signaling, and explicit unsupported statuses.
- `combat_victory_tests.c` characterizes the first combat victory and reward helpers: all-monsters-defeated detection, legacy-capped experience award application, level-increase metadata, bounded gold rewards, weapon/armour reward increments, no-reward reporting, invalid reward rejection, gold caps, and full-inventory reporting.
- `combat_monster_action_tests.c` characterizes bounded monster action resolution from `Sources/UltimaSpellCombat.c`: movement application after `FigureNewMonPosition`, archer shooting, magic targeting, poison, pilfer, armour miss checks, Exodus Castle auto-hit behavior, player damage, death handling, tile restoration, event signaling, and the first one-monster turn wrapper used by the modern shell.
...
- `dungeon_special_tile_tests.c` validates dungeon tile-effect contracts for strange wind, traps, gremlin food theft, writing/deferred tile reporting, invalid inputs, deterministic party-member selection, bounded HP/status mutation, food underflow prevention, prompt-backed fountain variants, mark bit/HP costs, Time Lord/message status, chest gold/trap/equipment rewards, and cancellation.
...
- `town_service_tests.c` validates the first bounded town healer service, including prompt/cancel handling, selected-character HP restoration, 200-gold cost mutation, insufficient-gold rejection, already-full and incapacitated states, invalid sessions, and unsupported commands.
