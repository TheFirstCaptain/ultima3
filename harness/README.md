...
- `dungeon_render_tests.c` validates the portable dungeon perspective frame contract from explicit dungeon bytes, including heading-relative sampling, near-to-far walls, doors, current-cell ladder/chest feature commands, invalid input fallback, and no source-buffer mutation.
