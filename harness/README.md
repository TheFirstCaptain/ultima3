...
- `combat_player_command_tests.c` characterizes the first bounded player combat command contract from `Sources/UltimaSpellCombat.c`: cardinal movement, blocked movement, pass, bounded flee success/failure, attack and Mittar direction prompting, melee hit/miss handoff, projectile and thrown-dagger line targeting, thrown-dagger inventory mutation, Mittar magic spend, spell invalid/insufficient-resource cases, all-party-defeated detection, redraw/audio signaling, and explicit unsupported statuses.
- `combat_victory_tests.c` characterizes the first combat victory and reward helpers: all-monsters-defeated detection, legacy-capped experience award application, and level-increase metadata.
