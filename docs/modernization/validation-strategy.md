...
| Dungeon perspective render contract | `Sources/UltimaDngn.c`: `DrawDungeon`, `DungeonBlock`, wall/door/ladder/chest drawing helpers | `harness/dungeon_render_tests.c` compiles `Core/src/u3_dungeon.c` and `Core/src/u3_render.c`; drives synthetic dungeon bytes and asserts heading-relative sampling, near-to-far wall and door command values, current-cell ladder/chest features, invalid-input fallback, and no source-buffer mutation. Swift adapter tests validate that Dungeon Doom sessions render through the dungeon frame path. | Harness and Swift tests passing; first renderer-neutral perspective frame extracted to `Core/` |
...
| Map/talk/combat fixture characterization | `Sources/UltimaMisc.c`: `LoadUltimaMap`; `Sources/UltimaSpellCombat.c`: `GetScreen` | `harness/resource_fixture_tests.c` uses resource parser fixtures for `MAPS`/`MONS`/`TLKS` 400 and 421, dungeon `MAPS`/`TLKS` 412, and `CONS` 400/410; asserts map size handling, map/monster/talk byte copies, dungeon byte copies, dungeon talk-table loading, full 200-byte combat-screen offsets, and the 121-byte special `CONS` limitation. | Harness passing; representative fixtures characterized |
