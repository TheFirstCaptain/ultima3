...
| Save/resource fixture extraction | `Sources/UltimaMisc.c`: `OpenRstr`, `GetParty`, `GetRoster`, `LoadUltimaMap`, `PutSosaria`, and mutable `MISC` table helpers | `harness/resource_fixture_tests.c` compiles `Core/src/u3_resource.c`; parses `Resources/English.lproj/MainResources.rsrc` as a read-only classic data-fork resource file and asserts the default save-template records and byte sizes used to create new roster files. | Harness passing; default templates characterized; changed user roster fixtures remain future work |
