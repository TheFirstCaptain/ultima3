...
Completed modernization work has established the harness, seeded portable C core modules, and characterized map/math behavior, Pascal strings, combat predicates, combat action resolution, autocombat behavior, dungeon navigation and perspective rendering, save/resource fixtures, resource inventory, persistence direction, representative map/talk/combat fixtures, shell resource/persistence integration, a byte-preserving native save domain, a fixed shell tick, persistent character creation and party assembly, manual save/load workflow, save-backed Sosaria movement and turn accounting, explicit location transitions, transient destination resource sessions, non-wrapping town/castle navigation, the first complete town/Sosaria round trip, bounded directional town NPC dialogue, the first town healer service, the first validated dungeon session entry, the first dungeon navigation/surface-return loop, bounded dungeon torch/light state, deterministic dungeon passive turn/encounter request metadata, the first non-interactive dungeon special tile effects, the first keyboard-prompt-backed dungeon interactive tile flows, transient combat session entry from dungeon encounter requests, the first resource-backed combat arena render, bounded player melee/ranged combat command slices, the first deterministic monster combat turn slice, combat victory return with experience and bounded loot awards, combat flee/defeat exits, dungeon chest trap/equipment reward outcomes, first-pass transient combat presentation overlays, and a bounded combat spell expansion beyond Mittar.
...
- `GameHostView` renders portable save/resource-backed Sosaria, location, dungeon, and combat frames through an AppKit renderer adapter, with synthetic and `CONS` fixture paths retained for fallback and validation.
- Keyboard, mouse, and menu commands pass through a portable input queue and are consumed by the shell tick; arrow and keypad commands drive wrapped Sosaria movement, non-wrapping town/castle movement, or dungeon forward/retreat/turn commands according to the active session.
...
- Inside town, `T` followed by a cardinal direction resolves an adjacent NPC from copied `MONS` data and displays a bounded printable line from the matching `TLKS` entry. Empty targets and unsupported entries produce explicit status without mutating the save. Pressing `H` starts the first bounded healer service: `1` through `4` select an active character, `0` or Escape cancels, successful healing restores HP to max for 200 gold from the selected roster member, and insufficient gold, full HP, incapacitated, invalid, and cancellation cases report explicit status without roster mutation.
- Explicit Enter at Dungeon Doom `(19,57)` loads validated `MAPS` and `TLKS` 412 records into a transient level-zero dungeon session at `(1,1)`, heading east. The shell retains the exact Sosaria return context, rejects Save in dungeon mode, and renders a portable first-person dungeon perspective frame from explicit dungeon bytes, level, position, heading, and light. While in a dungeon, movement commands use dungeon semantics: forward, retreat, left turn, right turn, space for Pass, `D` for Descend, and `K` for Klimb. `I` ignites a torch from the first living active party member with torch inventory, treating legacy good or poisoned status as living, sets light to the characterized legacy value, and marks the current document unsaved. Handled dungeon commands apply the shared move-counter cost, run portable passive turn mechanics, decay positive light, redraw the perspective frame when orientation, position, level, or light changes, report blocked/invalid ladder results without corrupting the active session, and restore the exact Sosaria frame and save-eligible party mode after a valid surface exit. Empty-floor random encounter requests now validate and copy the requested `CONS` combat screen into a transient combat session, snapshot party size and active roster ids, preserve the exact source dungeon session, render an 11x11 resource-backed combat arena with deterministic monster and party overlays, and keep Save rejected. Active combat now handles cardinal player movement, space/pass, `A` followed by arrow/keypad or `N`/`S`/`W`/`E` direction input for bounded melee, projectile weapon, and thrown dagger attacks, `M` followed by direction for Mittar, `U` followed by direction for Fulgar, `O` for Noxum, and `F` for a bounded flee attempt. Thrown dagger attacks decrement roster weapon inventory and unequip the dagger when the last one is thrown; Mittar spends 5 magic and line-targets the first monster, Fulgar spends 25 magic and line-targets fixed 75 damage, and Noxum spends 50 magic to damage all live monsters in the current bounded slice. Handled player movement/pass/attack/spell results and failed flee attempts run one deterministic monster turn unless the final monster is defeated. Monster turns include movement, melee hit/miss, poison, pilfer, shooting, and explicit deferred spell status. Combat frames can append transient presentation overlays for player and monster hit flashes, plus horizontal or vertical projectile/Mittar/Fulgar traces, derived from bounded combat results without changing turn rules or save data. Final-monster defeat applies the legacy experience award plus bounded gold and optional weapon/armour reward to the attacking roster member before restoring the preserved dungeon session; reward status reports gold caps, full inventories, no reward, and skipped cases. Successful flee restores the preserved dungeon session without XP or loot. If all active party members are defeated, the shell writes HP/status mutations to the in-memory document, discards transient combat state, restores the preserved dungeon session, and keeps Save rejected until the party returns to Sosaria. Monster HP/status/inventory mutations, thrown-dagger inventory mutations, supported combat spell magic/experience mutations, and victory reward mutations are written to the in-memory native document only; the remaining spell list remains deferred, and combat state remains transient. Non-interactive dungeon special tiles run through portable results: strange wind clears light, traps clear their transient tile and either disarm through the first living active party member or damage living members, gremlins clear their transient tile and steal food from a deterministic eligible living member, and writing decodes the current-level dungeon `TLKS` line into status text. Interactive dungeon tiles now use non-blocking keyboard prompts: fountains apply poison/heal/damage/cure variants based on `x & 3`, marks set the selected roster mark bit and charge HP, Time Lord tiles report bounded message status, and `G` opens current-cell dungeon chests for bounded gold, selected-character trap HP/status effects, and first weapon/armour inventory rewards. Party/roster mutations remain in the current in-memory native document until the party returns to Sosaria and the existing manual Save command succeeds; transient dungeon tile clearing and combat-session state are not written to the native save.
...
The next proposed tranche starts with bounded town vendors, representative destination coverage, overworld encounters/transport, and a final playability stabilization pass. See `F-061` through `F-064` in the feature tracker for the current proposed scope.
