...
- Explicit Enter at Dungeon Doom `(19,57)` loads validated `MAPS` and `TLKS` 412 records into a transient level-zero dungeon session at `(1,1)`, heading east. The shell retains the exact Sosaria return context, rejects Save in dungeon mode, and renders a portable first-person dungeon perspective frame from explicit dungeon bytes, level, position, heading, and light. While in a dungeon, movement commands use dungeon semantics: forward, retreat, left turn, right turn, `D` for Descend, and `K` for Klimb. `I` ignites a torch from the first living active party member with torch inventory, treating legacy good or poisoned status as living, sets light to the characterized legacy value, and marks the current document unsaved. Handled dungeon commands apply the shared move-counter cost, decay positive light, redraw the perspective frame when orientation, position, level, or light changes, report blocked/invalid ladder results without corrupting the active session, and restore the exact Sosaria frame and save-eligible party mode after a valid surface exit.
...
The next proposed backlog sequence starts with passive dungeon turn mechanics and encounter requests, then continues through special tile effects and interactive dungeon tile flows. See `F-045` through `F-047` in the feature tracker for the current proposed scope.
